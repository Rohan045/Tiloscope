import { LoaderCircle, Shield, UserRoundPen, X } from "lucide-react";
...
const ProfileBox = () => {
  ...
  const handleCheckboxChange = (event) => {
    setPassError(
      event.target.checked === true && passError === true ? true : false
    );
  ...
  const handlePassword = (e) => {
    ...
    if (
      isChecked &&
      (e.target.value !== formData.confPassword || e.target.value === "")
    ) {
      setPassError(true);
    } else {
      setPassError(false);
  ...
  const handleConfPassword = (e) => {
    ...
    if (
      isChecked &&
      (formData.password !== e.target.value || e.target.value === "")
    ) {
      setPassError(true);
    } else {
      setPassError(false);
  ...
  const updateProfile = async () => {
    try {
      const payload = {
        ...
        password:
          isChecked && formData.password !== "" ? formData.password : null,
      };
      setLoading(true);
      ...
      <div className="flex flex-col bg-[rgb(23,23,23)] p-3">
        ...
        {editable ? (
          <div className="flex flex-row justify-end">
            <div className="">
              <button
                className="bg-transparent border-solid border-zinc-700 border-[1px]"
                onClick={() => setProfileInfo(undefined)}
              >
                Discard
              </button>
            ...
            {editable ? (
              <div className="pl-2">
                <button
                  className={
                    "border-solid border-zinc-700 border-[1px]" +
                    (passError
