...
function Grid() {
  ...
  const DropArea = ({ onDrop, ele }) => {
    ...
    return (
      ...
      >
        <DraggableItem id={<Cell color="blue" />} ele={ele} />
      </div>
    );
  };
  ...
  const [toggle1x1, setToggle1x1] = useState(
    <DropArea onDrop={handleDrop1x1} />
  );

  const [toggle1x2, setToggle1x2] = useState(
    <DropArea onDrop={handleDrop1x2} />
  );
  ...
  const dragGridData = [
    ...
    <DraggableItem
      id={<Cell color="yellow" />}
      ele={<Cell color="yellow" />}
    />,
  ];
  return (
    <>
